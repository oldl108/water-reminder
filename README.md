...
> 作者微信公众号：[**爱玩的果果**](https://mp.weixin.qq.com/s/1P3FnoMtXusX0BdnAk4lGA)
...
- **早睡提醒**：到点提醒该睡觉了，按"第二天要不要上班"区分——周日～周四晚默认 22:00，
  周五、周六晚默认 22:30（可改）。点"知道了"当晚不再弹，忽略则每 30 分钟温和再催，
  凌晨 4 点后自动停。只提醒，不做记录
...
设置窗口右下角「下载最新版」：点击直接打开蓝奏云下载页，下载最新 zip 解压覆盖旧文件即可
（数据存在系统目录，不会丢）。

## 数据

全部存本地。程序平时不发任何网络请求（只有你手动点「检查更新」时才访问 GitHub 查询版本）：
